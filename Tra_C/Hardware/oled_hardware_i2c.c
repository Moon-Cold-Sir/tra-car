...
void oled_i2c_sda_unlock(void)
{
    ...
    do
    {
        ...
        if(DL_GPIO_readPins(GPIO_OLED_SDA_PORT, GPIO_OLED_SDA_PIN))
            break;
    }while(++cycleCnt < 100);
    mspm0_i2c_enable();
}
...
void OLED_WR_Byte(uint8_t dat,uint8_t mode)
{
    ...
    DL_I2C_fillControllerTXFIFO(OLED_INST, ptr, 2);
    DL_I2C_clearInterruptStatus(OLED_INST, DL_I2C_INTERRUPT_CONTROLLER_TX_DONE);
    ...
    while (!DL_I2C_getRawInterruptStatus(OLED_INST, DL_I2C_INTERRUPT_CONTROLLER_TX_DONE))
    {
...
void OLED_Init(void)
{
    if(DL_I2C_getSDAStatus(OLED_INST) == DL_I2C_CONTROLLER_SDA_LOW)
        oled_i2c_sda_unlock();
...
// void OLED_DisplayAngle(void)
// {
//     int roll_int = (int)wit_data.roll;
//     int pitch_int = (int)wit_data.pitch;
//     int yaw_int = (int)wit_data.yaw;
//     int temp_int = (int)wit_data.temperature;
//     // 显示 Roll（横滚角）
//     OLED_ShowString(2,4,(uint8_t *)"ro:",8);
//     if(roll_int < 0)
//     {
//         OLED_ShowString(22,4,(uint8_t *)"-",8);
//         roll_int=-roll_int;
//     }
//     else OLED_ShowString(22,4,(uint8_t *)" ",8);
//     OLED_ShowNum(30, 4, (uint32_t)roll_int, 3,8);
    
//     // 显示 Pitch（俯仰角）
//     OLED_ShowString(56,4,(uint8_t *)"pi:",8);
//     if(pitch_int < 0)
//     {
//         OLED_ShowString(74,4,(uint8_t *)"-",8);
//         pitch_int=-pitch_int;
//     }
//     else OLED_ShowString(74,4,(uint8_t *)" ",8);
//     OLED_ShowNum(82, 4, (uint32_t)pitch_int,4, 8);
    
//     // 显示 Yaw（偏航角）
//     OLED_ShowString(2,5,(uint8_t *)"ya:",8);
//     if(yaw_int < 0)
//     {
//         OLED_ShowString(22,5,(uint8_t *)"-",8);
//         yaw_int=-yaw_int;
//     }
//     else OLED_ShowString(22,5,(uint8_t *)" ",8);
//     OLED_ShowNum(30, 5, (uint32_t)yaw_int,4, 8);
    
//     // 显示温度
//     OLED_ShowString(56,5,(uint8_t *)"te:",8);
//     if(temp_int < 0)
//     {
//         OLED_ShowString(74,5,(uint8_t *)"-",8);
//         temp_int=-temp_int;
//     }
